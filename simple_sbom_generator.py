...
def parse_arguments():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='简化版SBOM生成器 - 为GitHub仓库生成SPDX 2.3格式的SBOM')
    
    parser.add_argument('--org', '-o', required=True, help='GitHub组织/用户名')
    parser.add_argument('--repo', '-r', required=True, help='GitHub仓库名')
    parser.add_argument('--output', '-f', default=None, help='输出文件路径 (默认: output/{org}_{repo}_sbom.json)')
    parser.add_argument('--token', '-t', help='GitHub个人访问令牌')
